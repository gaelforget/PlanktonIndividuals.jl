...
###########################################
respir_extra(Cq1) = respir_ex*(12.0e9*Cq1/Nn)^respir_b # extra cost for biosynthesis, related to Cq1
k_respir(size) = respir_a*(size)^respir_b # respiration rate, unit: per second
...
function PC(PAR, Temp, phyt) 
    ...
    PC = PCm*photoTempFunc*(1-exp(-PAR*phyt.chl/(phyt.Cq2*PCmax[phyt.sp]*photoTempFunc)))
    return PC
end
