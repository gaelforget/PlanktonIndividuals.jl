####################################### Parameters ########################################
PCmax = [1.5, 1.5] ./86400   # Maximum primary production rate (per second)
...
VNmax = [0.9, 0.9] ./ 86400 # Maximum N uptake rate (per second)
Nqmax = 0.5                 # Maximum N quota in cell (mmol N/mmol C)
Nqmin = 0.13                # Minimum N quota in cell (mmol N/mmol C)
KsatN = [0.005, 0.01]       # Half-saturation coeff
VN_b = [0.15, -0.2]         # Shape parameter

respir_a = 5.0e-7  # Respiration ratio 
respir_ex= 3.0e-8 # Extra cost of C for biosynthesis
...
k_sink = 0.1/86400  # Sink rates for agents (m/s)
