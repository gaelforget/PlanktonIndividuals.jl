...
PC_b = [0.15, 0.2]           # Shape parameter for size
#PC_ab= [-0.30, -0.30]       # Shape parameter for age
Chl2N = 3.0                  # Maximum Chla:N ratio in phytoplankton
R_NC  = 16/106               # N:C ratio in cell biomass, should be lower than 16:106
Cmin  = [2.0e-12, 2.0e-11]   # Minimum C quota (mmolC)
dvdcount = 0
...
Dor_P = 0.00                 # probability for a cell to do nothing in a time step
...
VN_b = [0.15, 0.2]          # Shape parameter for size
#VN_ab= [-0.30, -0.30]      # Shape parameter for age

respir_a = 0.05/86400 # Respiration ratio 
respir_ex= 3.0e-4   # Extra cost of C for biosynthesis
respir_b = 0.13      # Shape parameter for size
respir_ab= 0.20      # Shape parameter for age
